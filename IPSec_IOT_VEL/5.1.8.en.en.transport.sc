...
waitevent wait_nut_ping test_TEST-TAR-EN;
...
command test_TEST-TAR-EN TEST-TAR-EN {
    ...
    print "BASENAME> as described in test scenario 5.1.8.";
    ...
    execute "/bin/sh -c TEST-TAR-EN_CMD_PING6_FROM_SPEC_SRC_ADDR TEST-TAR-EN_IF1_PREFIX2_GA_2 -c 1 LOGO_IF1_PREFIX1_GA_2 | tee /tmp/5.1.8.TEST-TAR-EN_NAME.LOGO_NAME.result";
    delay 3;
    execute "/bin/sh -c TEST-TAR-EN_CMD_PING6_FROM_SPEC_SRC_ADDR TEST-TAR-EN_IF1_PREFIX2_GA_2 -c 4 LOGO_IF1_PREFIX1_GA_2 | tee -a /tmp/5.1.8.TEST-TAR-EN_NAME.LOGO_NAME.result";
    delay 1;
    execute "/bin/sh -c TEST-TAR-EN_CMD_UNAME_A | tee -a /tmp/5.1.8.TEST-TAR-EN_NAME.LOGO_NAME.result";
    delay 1;
    
    print "";
    print "BASENAME> The following ping6 is optional.";
    print "BASENAME> If YOUR DEVICE supports ping6, ";
    print "BASENAME> YOUR DEVICE transmits ICMPv6 Echo Request to TEST-TAR-EN.";
    print "BASENAME> ping6 TEST-TAR-EN_IF1_PREFIX2_GA_2 from LOGO_IF1_PREFIX1_GA_2";
    print "BASENAME> please save ping6 results to 5.1.8.LOGO_NAME.TEST-TAR-EN_NAME.result";
    print "BASENAME> Press Enter key for continue.";
    wait wait_nut_ping;
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "/bin/sh -c /usr/sbin/tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/5.1.8.TEST-TAR-EN_NAME.Link0.dump&";
    execute "/bin/sh -c /usr/sbin/tcpdump -i DUMPER_IF2 -c 10000 -s 0 -w /tmp/5.1.8.TEST-TAR-EN_NAME.Link1.dump&";
}
...
command setup_cleanup_REF2 REF2 {
    ...
    print "";
    print "5.1.8 (End-Node vs End-Node Transport Mode)";
    print "";
